...
| MCNX-BLD-01 | [build-and-integration](./build-and-integration.md) | fixed-string audit of `configuration.ccl` + a configure of the reference thornlist | B (unit) | exact | capability line `REQUIRES CarpetX WeakLibInterp` present; zero direct AMReX/HDF5/MPI requires and zero `PROVIDES` blocks; CST completes with zero unresolved capabilities |
| MCNX-BLD-02 | [build-and-integration](./build-and-integration.md) | source-tree scan for vendored AMReX/WeakLibInterp copies + symbol-level scan of the built executable | B (unit) | exact | zero vendored sources or nested builds; WeakLibInterp archive members present; AMReX symbols resolve to the single Cactus-provided install with zero duplicate-definition links |
| MCNX-BLD-03 | [build-and-integration](./build-and-integration.md) | source-convention audit (`src/make.code.defn` contents, tree scan) + environment-gated `AMREX_ENABLE_CUDA = yes` compile leg | B (unit) | exact | only `.cxx` sources listed; zero `.cu`/`.hip` files and zero secondary build descriptions; CUDA leg compiles unmodified or reports loud SKIP |
| MCNX-BLD-04 | [build-and-integration](./build-and-integration.md) | full Cactus harness run (`RunTest.pl`) over the reference configuration + aggregate family-B run + name cross-check against this matrix's family-A rows | A + B | exact + golden | every family-A row discovered and passing at `ABSTOL = RELTOL = 1e-12` (or its row-recorded override); family-B aggregate exits 0 with zero silent skips; no orphan tests |
| MCNX-BLD-05 | [build-and-integration](./build-and-integration.md) | CPU build of the reference configuration + startup smoke assertion + zero-iteration smoke parameter file | B (unit) | exact | build completes; `wli_value_type_size() == 8` startup assertion passes (abort on failure); smoke parameter file exits cleanly with no packets and no tables |

(The matrix above covers the complete corpus — every requirement id declared by every spec on disk; the closure rule keeps it complete under any future spec additions.)
...
- **The matrix covers the complete corpus (durable closure).** At any moment the matrix covers exactly the requirement ids of the specs on disk — now the full spec set, delivery (build-and-integration) included. The closure rule makes "complete" a checkable property, so any future spec addition extends the matrix without weakening the contract.
