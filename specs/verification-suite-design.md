...
- **The matrix grows with the corpus (by design, durable).** At any moment the matrix covers exactly the requirement ids of the specs on disk; rows for future specs (particle container, thorn integration, build) are appended when those specs land. The closure rule makes "complete" a checkable property at every phase, so this growth does not weaken the contract.
