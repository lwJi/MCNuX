...
- `mcnux_interactions.cxx` — runtime episode driver (`MCNuX_EpisodeDriver`, gated `enable_interactions`, IN MCNuX_TransportStep): per-packet episode loop with frozen kinematics, scattering event application via `scatter_redraw` + C2 deposit, `interaction_step_audit()` owner; documents the driver-local cell-boundary conventions (direction-aware entry adjustment, 4·eps exit-bound inflation).
...
- `MCNuX/test/` is the Cactus test home: `test.ccl` (`NPROCS 1`), `<name>.par` parfiles, `<name>/` golden dirs. Current tests: `unit-selftest`, `source-zero-add`, `minkowski-freestream`, `schwarzschild-id`, `hydro-sphere-id`, `hydro-sphere-gather`, `emit-smoke`, `interactions-fixedseed`.
