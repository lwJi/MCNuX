...
- [x] T2: Philox4x32-10 RNG primitive — pure device-callable `u(S,q,e,k)`: round function + constants ([MCNX-RNG-01]), counter/key packing ([MCNX-RNG-03]), `[0,1)` double via `(u64 >> 11) * 2^-53` ([MCNX-RNG-04]); header wired into make.code.defn
  ...
  - notes: DONE — `MCNuX/src/mcnux_rng.hxx` (header-only, `namespace MCNuX`, `<cstdint>` only): `philox4x32_10` block fn, `philox_input` packing, `uniform_from_u64`, public `constexpr double u(S,q,e,k)`. All three KATs + `[0,1)` closed forms + purity/permutation/pairing checks are static_asserts, re-verified every build via `stub.cxx` include; no `make.code.defn` change needed (headers reach the build through `stub.cxx` — now the standing aggregation point: future shared headers add one `#include` there). Device-callability is plain `constexpr` via nvcc `--expt-relaxed-constexpr` (no `AMREX_GPU_HOST_DEVICE`; annotating would force an AMReX include). Host-vs-device bitwise leg of [MCNX-RNG-06] deferred to T25d (recorded in header comment `:32-40`; see Discovered). Minor spec-wording nit, no action: `:134` says no counter-based RNG exists in the reference stack, but AMReX's SYCL branch names an opaque `mkl::rng::device::philox4x32x10<>` (`amrex/Src/Base/AMReX_RandomEngine.H:33`) — unusable as exemplar, pinning rationale stands.
- [x] T3: Paramcheck guard on subcycling — `MCNuX/param.ccl`: `SHARES: Driver` + `USES BOOLEAN use_subcycling` (idiom `CarpetX/ODESolvers/param.ccl:23-27`); `CCTK_PARAMCHECK` routine aborting with explanatory error when `yes` ([MCNX-CTX-06])
  ...
  - notes: DONE — `MCNuX/src/mcnux_paramcheck.cxx` (`extern "C" MCNuX_ParamCheck`, `CCTK_VERROR` on `use_subcycling`), `SCHEDULE ... AT CCTK_PARAMCHECK` with `OPTIONS: global`, no interface.ccl change needed. Wording fix vs original plan: `SHARES: CarpetX` does not bind (no such capability); the driver `IMPLEMENTS: Driver` (`CarpetX/CarpetX/interface.ccl:3`), so `SHARES: Driver` is the working, spec-cited form (`specs/carpetx-thorn-integration.md:82-84`) — verified live. Exact criterion enforced by the new non-harness gate `agent_scripts/gate_paramcheck.sh` + twin parfiles `agent_scripts/gates/subcycling_{yes,no}.par` (aborting runs can't be harness tests, `specs/verification-suite-design.md:249-253`); precedent for T25b/T25c. Empirical parfile facts every future parfile inherits: `Cactus::presync_mode = "mixed-error"` is required to survive CarpetX's own paramcheck (which runs before MCNuX's), and `CarpetX::restrict_during_sync = no` is required whenever subcycling is on. Spec anchor `:161-166`.
- [x] T4: Transport cadence schedule group — MCNuX-owned group `AT evol AFTER ODESolvers_Solve` in `MCNuX/schedule.ccl` with an initial observable routine (step counter/diagnostic), `OPTIONS: level` (or `global`), complete `READS:`/`WRITES:`; driver-include wiring to reach `ghext` ([MCNX-CTX-01/03/04/05])
  - spec: specs/carpetx-thorn-integration.md
  ...
  - notes: DONE — `MCNuX/src/mcnux_cadence.cxx` (init + observer routines), checkpointed grid scalar `mcnux_transport_diag/transport_step_count` in `interface.ccl`, `SCHEDULE GROUP MCNuX_TransportStep AT evol AFTER ODESolvers_Solve` + `MCNuX_TransportObserver` (`OPTIONS: global` — chosen over `level`: once per traversal = once per coarsest Δt under the `use_subcycling=no` pin; exemplar `CarpetX/TestOutput/schedule.ccl:54-82`) + `MCNuX_TransportInit AT initial` (makes the observer's `READS:` valid under `presync_mode="mixed-error"`). Cadence verified empirically: exactly one observer firing per iteration over 10 RK4 steps, `t` advancing by one coarsest Δt, never per RK stage; 64-box multi-box rerun still 10 firings (global = per traversal, not per tile). ghext reachability proven live (null-check + `patchdata.size()`/`leveldata.size()` in the CCTK_VINFO line) via `#include "../../../CarpetX/CarpetX/src/driver.hxx"` — three `../`, MCNuX being its own arrangement (one deeper than the ODESolvers exemplar). Standing build fact: `REQUIRES CarpetX` alone suffices to include driver internals (make.CARPETX.defn transitively provides AMREX/ARITH/LOOP/YAML_CPP include dirs) — never add `REQUIRES AMReX`; installing `driver.hxx` as a bindings header would break (sibling `valid.hxx` not installed), the relative include is the only route. `gate_paramcheck.sh` still green; all cadence-check artifacts transient under `$CACTUSX/GATE/mcnux/`, no third permanent gate added (`specs/verification-suite-design.md:249-253`). Recovery caveat (not exercised): if CarpetX doesn't mark recovered scalars valid under mixed-error, schedule the init routine also `AT CCTK_POST_RECOVER_VARIABLES`.
- [x] T5: First benchmark scaffolding — create `MCNuX/test/test.ccl` (`NPROCS 1`, precedent `CarpetX/TestODESolvers/test/test.ccl:1-16`), `run_selftests` parameter, selftest battery routine writing pass/fail TSV, `unit-selftest.par` + golden dir ([MCNX-VER-01], [MCNX-BLD-05])
  ...
  - notes: DONE — harness now 1 test discovered + passing (`unit-selftest`, 2 files identical at defaults; gate_paramcheck + validate_specs still green). Battery: `MCNuX/src/mcnux_selftest.cxx`, 36 append-only rows (index→name map at `:20-53`) writing `mcnux_selftest` = `CCTK_REAL TYPE=ARRAY DIM=1 SIZE=36 DISTRIB=CONSTANT {selftest_error, selftest_pass}` — `TYPE=ARRAY` risk retired: CarpetX accepts it and `WriteTSVArrays` emits one row per index (only a `.x.tsv` file). Golden `MCNuX/test/unit-selftest/mcnux-mcnux_selftest.it000000.x.tsv` copied verbatim from a sandbox run. Later tasks append rows at the END, bump `SIZE=` in `interface.ccl`, and regenerate the golden file; routine hard-errors on table/SIZE mismatch. T1/T2 fixtures were refactored runtime-reachable (`pinned::` anchors + `species_table_holds()` in `mcnux_units.hxx`; `kat_holds(i)`/`uniform_closed_form_holds(i)` tables in `mcnux_rng.hxx`) — consistency sweep still holds, each literal appears once. Parfile facts every later benchmark inherits: `IO::out_fileinfo = "axis labels"` (else IOUtil embeds date/hostname in TSVs, dirtying goldens), `IO::out_dir = $parfile`, `IO::parfile_write = no`, `CarpetX::out_metadata = no`, `ActiveThorns = "CarpetX IOUtil MCNuX"` suffices for Minkowski. `run_selftests` is `RESTRICTED BOOLEAN`, default `no`, guarded by `if (run_selftests) { STORAGE + SCHEDULE AT initial }`. Deferred: `sizeof(amrex::Real/ParticleReal)==8` rows → T8; other spec battery legs append with their owning tasks. Caveat: row 16 (opacity factor) has measured error 7.9e-13 < ABSTOL 1e-12, so the golden diff alone can't discriminate that row's error column; its pass column + compile-time assert cover it.
- [x] T25a: Run `bash specs/tools/validate_specs.sh` in the sandbox once to confirm the corpus passes
  - spec: specs/build-and-integration.md
  - tests: validator exit 0 is the test (exact).
  - notes: DONE — validator green on first run inside the sandbox: exit 0, banner `ALL CHECKS PASSED (2026-08-14T15:38:22Z)`, 219 ok / 0 fail, all five reference roots resolved (flesh via `MCNX_FLESH_ROOT=$CACTUSX`). Corpus is exactly 13 spec files, README links them all, no orphans. Sandbox-only for one reason: the `flesh/` root resolves solely through `MCNX_FLESH_ROOT` (persisted by `agent_scripts/sandbox/setup.sh`); outside the sandbox every `flesh/...` citation check fails spuriously — environment defect, never a corpus defect. Re-run cost ~seconds; cheap to repeat after any spec-touching task.
...
- INHERITS vs bare READS: whether `READS:` on `ADMBaseX::`/`HydroBaseX::` groups requires `INHERITS:` in `interface.ccl` under CarpetX flesh — moot for T4 (own variable needs no `INHERITS:`); still open, T15/T17 implementer determines empirically at first cross-thorn READS.
...
- [ ] T25d acceptance delta: complete [MCNX-RNG-06]'s host-vs-device leg in the GPU build — evaluate `MCNuX::u` on device and require bitwise equality with host values for the `mcnux_rng.hxx` `detail::purity_batch` tuples (deferral recorded in the header comment `mcnux_rng.hxx:32-40`).
  - spec: specs/rng-and-statistical-acceptance.md
  - tests: device-vs-host bitwise equality over the purity-batch tuples (exact), run in the T25d GPU-backend build.
  - notes: discovered 2026-08-14 during T2; the CPU/MPI sandbox cannot exercise this leg, so T2 shipped with all host-side purity legs asserted at compile time. Next plan run should fold this into T25d's scope.
