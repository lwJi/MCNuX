...
- **Fluid-gather reuse (C1):** `make_fluid_gather`/`HydroGroups`/`for_each_packet_tile[_raw]` in `mcnux_geodesic.cxx:117-206` are used only by the diagnostic `MCNuX_FluidGatherDiag` (`mcnux_geodesic.cxx:461-531`). The emission loop and episode driver must reuse these helpers, not duplicate them — factor into a shared header if needed.
- **Atomic deposition idiom (C2):** [MCNX-GPU-02]'s pinned exemplar (`ParallelFor` over packet tile + `amrex::Gpu::Atomic::AddNoRet` into `Array4`) has no production instance. `emit-core`, `episode-driver`, and `absorb-remove` all need it; build it ONCE as a shared helper wired to the ledger pure functions `emission_delta`/`absorption_delta`/`scattering_delta` (`mcnux_srcterms.hxx:62-87`, currently uncalled from any event path).
- **Selftest growth:** the index→name row table in `mcnux_selftest.cxx` is APPEND-ONLY; new checks go in per-domain `mcnux_selftest_<domain>.cxx` files. Rows ≥128 requires bumping `interface.ccl:51` SIZE=128 and re-capturing the `unit-selftest` golden TSV.
...
- [ ] [ledger-closure] Conservation-ledger closure diagnostic ([MCNX-HYD-05]) — buildable now, no dependencies
  ...
  - notes: natural home alongside the C2 deposition helper; every later deterministic benchmark also wires it in (`verification-suite-design.md:393`); `interactions-fixedseed` consumes it.
- [ ] [emit-bins] Emission runtime pure helpers: energy-bin grid + uniform→physical mapping
  ...
  - notes: (a) bin grid (E_lo/E_hi/center per bin b) that `bin_center_energy`/`bin_integrated_eta` (`mcnux_emission.hxx:91-93,131`) consume — coordinate with `mcnux_table_coeffs.hxx` energy-grid handling; open question: whether `bin_integrated_eta` is called once per (bin, cell, step) and where edges are pinned — settle here. (b) map `CreationUniforms` (`mcnux_emission.hxx:186-209`): `u_pos_{x,y,z}` → cell-bounds coordinates, `u_time` → [t_n, t_n+Δt) per [MCNX-PKT-03] (`specs/packet-representation-and-sampling.md:107-117`; coordinate volume, not proper volume).
- [ ] [emit-core] Runtime emission/creation loop (`mcnux_emission.cxx`, scheduled IN MCNuX_TransportStep)
  - spec: specs/packet-representation-and-sampling.md
  ...
  - notes: depends on emit-bins; uses C1+C2. Per (cell, species, bin): gather fluid+metric (√−g, ΔV), η_b(s) via RangedTableCoefficients dispatch + `bin_integrated_eta`; `packet_count` → `cell_count_uniform` (Bernoulli at (q=K_cell, e=n, k=3b+s)) → `emission_count`; per packet draw `creation_uniforms`, map via emit-bins, `fluid_frame_direction` → `build_tetrad` → `transform_to_coordinate_frame` (`mcnux_tetrad.hxx:216-323`), N = `packet_weight(E_p, bin_center_energy)`, append SoA row with fresh unique `idcpu` per [MCNX-GPU-04] (unique across run/ranks, never reused, bit-63 clear — design the allocation scheme here; fixture hand-assigns `SetParticleIDandCPU(i+1,0)` at `mcnux_geodesic.cxx:378`). Deposit `emission_delta` via C2. g=4 exactly-once (`mcnux_emission.hxx:107`). `E_p` param exists (`param.ccl:104-117`). Keep `MCNuX_SeedSyntheticPackets` as a parameter-gated fixture.
